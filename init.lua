...
--
-- load = function(plug)  →  custom load; call vim.cmd('packadd '..plug.name)
-- load = false           →  never auto-loaded; packadd called manually as dep
-- PackChanged            →  fires after install/update (use for build steps)
-- ============================================================================
...
vim.pack.add {
...

  {
    ...
    load = function(plug)
      -- g: vars must be set before packadd
  ...
  -- Dep: must be on rtp before nvim-treesitter.setup()
  { name = 'nvim-treesitter-textobjects', src = 'https://github.com/nvim-treesitter/nvim-treesitter-textobjects', load = false },
...

  {
    ...
    load = function(plug)
      ...
      vim.api.nvim_create_autocmd('BufWritePre', { once = true, callback = setup })
...

  {
    ...
    load = function(plug)
      ...
      -- Stub deleted after first call so fzf-lua's real FzfLua command takes over
      ...
      vim.api.nvim_create_autocmd('LspAttach', { once = true, callback = setup })
    end,
  },
  ...
  -- ── CodeCompanion commands (plenary + treesitter are flat deps) ─────────────

  { name = 'plenary.nvim', src = 'https://github.com/nvim-lua/plenary.nvim', load = false },

  {
    ...
    load = function(plug)
      ...
      local function setup()
        ...
        vim.cmd 'packadd nvim-treesitter'  -- already loaded after first buf; safe to repeat
        vim.cmd('packadd ' .. plug.name)
  ...
  -- ── DAP commands (nvim-dap + flat deps) ─────────────────────────────────────

  { name = 'nvim-nio',              src = 'https://github.com/nvim-neotest/nvim-nio',             load = false },
  { name = 'nvim-dap-ui',           src = 'https://github.com/rcarriga/nvim-dap-ui',              load = false },
  { name = 'nvim-dap-virtual-text', src = 'https://github.com/theHamsta/nvim-dap-virtual-text',   load = false },
  { name = 'nvim-dap-python',       src = 'https://github.com/mfussenegger/nvim-dap-python',      load = false },
