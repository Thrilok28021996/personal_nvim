-- ============================================================================
-- KEYMAPS CONFIGURATION (Neovim 0.12+)
-- ============================================================================
--
...
--   4. Code Features (diagnostics, quickfix, spell, refactoring)
--   5. Development Tools (debugging, terminal, execution)
--   6. Visual & UI Toggles
--   7. Sessions & Project Management
--   8. Plugin Integrations (LSP, AI, git, formatting, etc.)
--
-- Native defaults NOT remapped here (use :help for reference):
--   Folding: za zA zo zO zc zC zm zM zr zR zf zd zE zj zk
--   Diff: ]c [c do dp
--   Search: gn gN g* g#
--   Spell: zg zw
--   URL: gx (0.12+ opens URLs + LSP documentLinks + help online docs)
--   LSP: grn grr gri gra grt grx K <C-S> [d ]d
--   Treesitter: v_an v_in v_]n v_[n (incremental selection 0.12+)
-- ============================================================================
...
map('n', '<leader>da', function()
  vim.diagnostic.config {
    virtual_text = { prefix = '●', source = 'if_many', spacing = 4, current_line = true },
    signs = true,
    underline = true,
  }
...
map('n', '<leader>xq', function() toggle_list('quickfix', 'copen', 'cclose') end, { desc = 'Toggle quickfix list' })
map('n', '<leader>xl', function() toggle_list('loclist', 'lopen', 'lclose') end, { desc = 'Toggle location list' })
...
-- 4.4 LSP Code Actions & Refactoring
...
map('n', '<leader>rr', function()
  ...
  local runners = {
    ...
    lua = 'lua ', sh = 'bash ', bash = 'bash ', ruby = 'ruby ',
    go = 'go run ', rust = 'cargo run',
...
map('n', '<leader>tu', '<cmd>Undotree<CR>', { desc = 'Toggle undo tree (0.12+)' })
map('n', '<leader>tD', '<cmd>DiffTool<CR>', { desc = 'Open diff tool (0.12+)' })
...
-- 7.2 Project Management (vim.fs.root 0.12+ equal-priority)
...
-- ============================================================================
-- SECTION 8: PLUGIN INTEGRATIONS
-- ============================================================================

-- 8.1 Plugin Manager & LSP Management
map('n', '<leader>L', '<cmd>Lazy<CR>', { desc = 'Open Lazy plugin manager' })
map('n', '<leader>li', '<cmd>lsp<CR>', { desc = 'LSP: Interactive manager (0.12+)' })

-- 8.2 Task Runner (native :make)
...
vim.api.nvim_create_autocmd('FileType', {
  ...
  callback = function()
    ...
    vim.keymap.set('v', '<C-b>', function() vim.cmd 'normal! `<i**`>la**' end, vim.tbl_extend('force', opts, { desc = 'Bold selection' }))
    vim.keymap.set('v', '<C-i>', function() vim.cmd 'normal! `<i*`>la*' end, vim.tbl_extend('force', opts, { desc = 'Italic selection' }))

    vim.keymap.set('n', '<leader>il', function()
      local text, url = vim.fn.input 'Text: ', vim.fn.input 'URL: '
      if url ~= '' then vim.api.nvim_put({ string.format('[%s](%s)', text, url) }, 'c', true, true) end
...
vim.api.nvim_create_autocmd('LspAttach', {
  ...
  callback = function(args)
    ...
    -- Native LSP completion (0.12+ autocomplete option handles triggering)
    ...
    -- Workspace diagnostics (0.12+ pull diagnostics)
    if client:supports_method 'workspace/diagnostic' then
      lmap('n', '<leader>dW', vim.lsp.buf.workspace_diagnostics, 'Workspace diagnostics (pull)')
    end
    ...
    -- Code Lens (0.12+ displays as virtual lines)
    ...
    -- Document color (0.12+ — inline color previews)
    if client:supports_method 'textDocument/documentColor' then
      vim.lsp.document_color.enable(true, buf)
    end

    -- Linked editing range (0.12+ — e.g. rename matching HTML tags)
...
-- 8.6 AI Assistant (CodeCompanion)
...
-- 8.8 Git Links (native — replaces gitlinker.nvim)
...
-- 8.10 Autoformat (Conform)
map({ 'n', 'v' }, '<leader>cf', function()
  require('conform').format { async = true, lsp_fallback = true }
end, { desc = 'Format buffer' })

-- 8.11 Fuzzy Finder (fzf-lua)
map('n', '<leader>fh', '<cmd>FzfLua helptags<cr>', { desc = 'Search help tags' })
map('n', '<leader>fk', '<cmd>FzfLua keymaps<cr>', { desc = 'Search keymaps' })
map('n', '<leader>fd', function() require('fzf-lua').diagnostics_document() end, { desc = 'Search diagnostics (file)' })
map('n', '<leader>fD', function() require('fzf-lua').diagnostics_workspace() end, { desc = 'Search diagnostics (workspace)' })
...
-- 8.12 Gitsigns (buffer-local, set via on_attach)
